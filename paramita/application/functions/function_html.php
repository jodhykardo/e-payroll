...
function create_button2($text, $color, $onclick, $icon, $size='md'){
	echo '<button class="btn btn-'.$color.' btn-'.$size.'" style="position:relative;bottom:40px" onclick="'.$onclick.'"><i class="glyphicon glyphicon-'.$icon.'"></i> '.$text.'</button>';
}
...
function create_table2($column, $content=""){
   echo'<hr/><div class="table-responsive" style="position:relative;bottom:60px">
   <table class="table table-striped" width="100%">
   <thead><tr>
   <th style="width: 10px">No</th>';
...
function start_modal2($id, $action=''){
	echo '<div class="modal" id="'.$id.'" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static">
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
	  
   <form class="form-horizontal" method="post" onsubmit="'.$action.'" enctype="multipart/form-data">
   <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"> &times; </span> </button>
      <h3 class="modal-title"></h3>
   </div>
...
function end_modal2(){
   ...
   <div class="modal-footer">
      <input type="submit" value="Upload" class="btn btn-primary btn-save" name="submit"> 
		<button type="button" class="btn btn-warning" data-dismiss="modal">  	 
	    <i class="glyphicon glyphicon-remove-sign"></i> Batal
	  </button>
   </div>
...
function form_combobox2($label, $name, $list, $width='5', $class="", $attr=""){
   echo'<div class="form-group" style="position:relative;bottom:34px;left:585px">
   <label for="'.$name.'" class="col-sm-2 control-label"> '.$label.'</label>
   <div class="col-sm-'.$width.'">
      <select class="form-control '.$class.'" name="'.$name.'" id="'.$name.'" '.$attr.'>
         <option value="">- Pilih -</option>';
