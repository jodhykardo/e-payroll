...
<?php
   create_table(array("Perusahaan", "Nama Karyawan", "Telat 5 > Menit", "Telat 30 > menit", "Telat > 1 Jam", "Telat Istirahat","Alpa","Sakit","Izin","Kehadiran"));
end_content();
...
$(function(){
   ...
   //Menerapkan plugin dataTable dengan tambahan tombol ekspor
	  ...
	  table = $('.table').DataTable({
        ...
        "processing" : true,
        "ajax" : {
          "url" : "<?= BASE_PATH ?>/admin/laporanabsen/listData/"+mulai+"_"+sampai,
          "type" : "POST"
        }
      });
   ...
   $('#form-laporan').submit(function(){
      mulai = $('#mulai').val();
      sampai = $('#sampai').val();
      document.title = "Laporan Penggajian Dari "+mulai+" Sampai "+sampai;
      table.ajax.url("<?= BASE_PATH ?>/admin/laporanabsen/listData/"+mulai+"_"+sampai);
      table.buttons();
      table.ajax.reload();
      return false;
    });
