...
<?php
   create_table(array("Perusahaan","Nama Karyawan", "Tanggal", "Jam Masuk", "Jam Istirahat","Jam Masuk Siang", "Jam Pulang"));
end_content();
...
$(function(){
   ...
   //Menerapkan plugin dataTable dengan tambahan tombol ekspor
      table = $('.table').DataTable({
        ...
        "processing" : true,
        "ajax" : {
          "url" : "<?= BASE_PATH ?>/admin/absen/listData/"+mulai+"_"+sampai,
          "type" : "POST"
        }
      });
   ...
   $('#form-laporan').submit(function(){
      mulai = $('#mulai').val();
      sampai = $('#sampai').val();
      document.title = "Rekap Absensi Dari "+mulai+" Sampai "+sampai;
      table.ajax.url("<?= BASE_PATH ?>/admin/absen/listData/"+mulai+"_"+sampai);
      table.buttons();
      table.ajax.reload();
      return false;
    });
